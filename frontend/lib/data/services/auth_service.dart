...
class AuthService {
  ...
  void _handleDioError(DioException e, String context) {
    ...
    if (e.type == DioExceptionType.connectionTimeout ||
      ...
      throw Exception(
          'Server is taking too long to respond. It might be waking up, please try again in a few seconds.');
    }

    if (e.type == DioExceptionType.connectionError) {
      throw Exception(
          'Cannot reach the server. Please check if your internet or DNS is working correctly.');
    }

    if (e.error != null && e.error.toString().contains('SocketException')) {
      throw Exception(
          'Network unreachable. Please check your data connection or Wi-Fi.');
    }

    throw Exception('Server communication error. Please try again later.');
  }

  Future<User> _saveAuthData(
    ...
    final avatarUrl =
        (userData['avatarUrl'] ?? userData['avatar_url']) as String?;
  ...
  Future<void> logout() async {
    try {
      ...
      //   debugPrint("🚩 Background database wipe failed: $e");
      // }));
    } catch (e) {
