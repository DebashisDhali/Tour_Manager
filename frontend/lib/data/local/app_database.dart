...
class Users extends Table {
  ...
  TextColumn get purpose =>
      text().nullable()(); // 'tour', 'mess', 'event', 'business'
...
class TourMembers extends Table {
  ...
  RealColumn get mealCount =>
      real().withDefault(const Constant(0.0))(); // For Mess sessions
  DateTimeColumn get leftAt => dateTime().nullable()();
  TextColumn get status => text().withDefault(const Constant('active'))();
  TextColumn get role => text()
      .withDefault(const Constant('viewer'))(); // 'admin', 'editor', 'viewer'
  BoolColumn get isSynced => boolean().withDefault(const Constant(false))();
  BoolColumn get isDeleted => boolean().withDefault(const Constant(false))();
...
class Expenses extends Table {
  TextColumn get id => text()();
  TextColumn get tourId => text().references(Tours, #id)();
  TextColumn get payerId =>
      text().named('payer_id').nullable().references(Users, #id)();
  RealColumn get amount => real()();
  TextColumn get title => text()();
  TextColumn get category => text()();
  TextColumn get messCostType =>
      text().nullable()(); // 'fixed' or 'meal' (bazar)
  BoolColumn get isSynced => boolean().withDefault(const Constant(false))();
  BoolColumn get isDeleted => boolean().withDefault(const Constant(false))();
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
        ...
        beforeOpen: (details) async {
          ...
          await safePatch(
              'UPDATE users SET purpose = "tour" WHERE purpose IS NULL');
          await safePatch('UPDATE users SET is_me = 0 WHERE is_me IS NULL');
          await safePatch(
              'UPDATE users SET name = "User" WHERE name IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE tours SET created_by = "" WHERE created_by IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE tours SET name = "Tour" WHERE name IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE tour_members SET status = "active" WHERE status IS NULL');
          await safePatch(
              'UPDATE tour_members SET role = "viewer" WHERE role IS NULL');
          await safePatch(
              'UPDATE tour_members SET meal_count = 0.0 WHERE meal_count IS NULL');
          await safePatch(
              'UPDATE expenses SET created_at = $now WHERE created_at IS NULL');
          await safePatch(
              'UPDATE expenses SET amount = 0.0 WHERE amount IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE expenses SET title = "Expense" WHERE title IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE expenses SET category = "Other" WHERE category IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE expense_splits SET amount = 0.0 WHERE amount IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE expense_payers SET amount = 0.0 WHERE amount IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE settlements SET date = $now WHERE date IS NULL');
          await safePatch(
              'UPDATE settlements SET amount = 0.0 WHERE amount IS NULL'); // Exhaustive
          await safePatch(
              'UPDATE program_incomes SET date = $now WHERE date IS NULL');
          await safePatch(
              'UPDATE program_incomes SET amount = 0.0 WHERE amount IS NULL'); // Exhaustive
          await safePatch(
          ...
          for (var table in tablesWithIsDeleted) {
            await safePatch(
                'UPDATE $table SET is_deleted = 0 WHERE is_deleted IS NULL');
            await safePatch(
                'UPDATE $table SET is_synced = 0 WHERE is_synced IS NULL');
          }
        },
        onUpgrade: (m, from, to) async {
          ...
          if (from < 6) {
            ...
            await customStatement(
                'UPDATE users SET is_me = 1 WHERE id IN (SELECT id FROM users LIMIT 1)');
          }
          if (from < 7) {
          ...
          if (from < 20) {
            ...
            await runSqlSafe(
                'ALTER TABLE tour_members ADD COLUMN is_deleted INTEGER NOT NULL DEFAULT 0');
            await runSqlSafe(
                'ALTER TABLE meal_records ADD COLUMN is_deleted INTEGER NOT NULL DEFAULT 0');
            ...
            await customStatement(
                'UPDATE tours SET updated_at = $now WHERE updated_at IS NULL');
            await customStatement(
                'UPDATE tours SET is_deleted = 0 WHERE is_deleted IS NULL');
            await customStatement(
  ...
  Future<void> createTour(Tour tour) =>
      into(tours).insert(tour, mode: InsertMode.insertOrReplace);

  // Expenses with Transaction
  Future<void> addExpenseWithDetails(
    ...
    return transaction(() async {
      ...
      for (final split in splits) {
        await into(expenseSplits)
            .insert(split, mode: InsertMode.insertOrReplace);
      }
      for (final payer in payers) {
        await into(expensePayers)
            .insert(payer, mode: InsertMode.insertOrReplace);
      }
    });
  }

  Future<void> updateExpenseWithDetails(
      Expense expense, List<ExpenseSplit> splits, List<ExpensePayer> payers) {
    return transaction(() async {
      await into(expenses).insert(expense, mode: InsertMode.insertOrReplace);
  ...
  Future<void> hardDeleteExpenseWithDetails(String expenseId) {
    return transaction(() async {
      await (delete(expenseSplits)..where((t) => t.expenseId.equals(expenseId)))
          .go();
      await (delete(expensePayers)..where((t) => t.expenseId.equals(expenseId)))
          .go();
  ...
  Future<void> createSettlement(Settlement settlement) =>
      into(settlements).insert(settlement, mode: InsertMode.insertOrReplace);
  Future<void> deleteSettlement(String id) =>
      (update(settlements)..where((t) => t.id.equals(id))).write(
          const SettlementsCompanion(
              isDeleted: Value(true), isSynced: Value(false)));
  Future<void> hardDeleteSettlement(String id) =>
      (delete(settlements)..where((t) => t.id.equals(id))).go();

  Future<void> createProgramIncome(ProgramIncome income) =>
      into(programIncomes).insert(income, mode: InsertMode.insertOrReplace);
  Future<void> deleteProgramIncome(String id) =>
      (update(programIncomes)..where((t) => t.id.equals(id))).write(
          const ProgramIncomesCompanion(
              isDeleted: Value(true), isSynced: Value(false)));
  Future<void> hardDeleteProgramIncome(String id) =>
      (delete(programIncomes)..where((t) => t.id.equals(id))).go();

  Future<void> deleteTourWithDetails(String tourId) {
    return transaction(() async {
      await (update(tours)..where((t) => t.id.equals(tourId))).write(
          const ToursCompanion(isDeleted: Value(true), isSynced: Value(false)));
      await (update(expenses)..where((t) => t.tourId.equals(tourId))).write(
          const ExpensesCompanion(
              isDeleted: Value(true), isSynced: Value(false)));
      await (update(expenseSplits)
            ..where((s) => s.expenseId.isInQuery(selectOnly(expenses)
              ..addColumns([expenses.id])
              ..where(expenses.tourId.equals(tourId)))))
          .write(const ExpenseSplitsCompanion(
              isDeleted: Value(true), isSynced: Value(false)));
      await (update(expensePayers)
            ..where((p) => p.expenseId.isInQuery(selectOnly(expenses)
              ..addColumns([expenses.id])
              ..where(expenses.tourId.equals(tourId)))))
          .write(const ExpensePayersCompanion(
              isDeleted: Value(true), isSynced: Value(false)));
      await (update(settlements)..where((t) => t.tourId.equals(tourId))).write(
          const SettlementsCompanion(
              isDeleted: Value(true), isSynced: Value(false)));
      await (update(programIncomes)..where((t) => t.tourId.equals(tourId)))
          .write(const ProgramIncomesCompanion(
              isDeleted: Value(true), isSynced: Value(false)));
      await (delete(syncMetadata)
            ..where((m) => m.key.equals('$_localOnlyTourKeyPrefix$tourId')))
          .go();
    });
  }
  ...
  Future<void> hardDeleteTourWithDetails(String tourId) {
    return transaction(() async {
      ...
      final tourExpenses =
          await (select(expenses)..where((t) => t.tourId.equals(tourId))).get();
      ...
      await (delete(programIncomes)..where((t) => t.tourId.equals(tourId)))
          .go();
  ...
  Future<List<User>> getUnsyncedUsers() =>
      (select(users)..where((t) => t.isSynced.equals(false))).get();
  Future<List<Tour>> getUnsyncedTours() =>
      (select(tours)..where((t) => t.isSynced.equals(false))).get();
  Future<List<Expense>> getUnsyncedExpenses() =>
      (select(expenses)..where((t) => t.isSynced.equals(false))).get();
  Future<List<ExpenseSplit>> getUnsyncedSplits() =>
      (select(expenseSplits)..where((t) => t.isSynced.equals(false))).get();
  Future<List<ExpensePayer>> getUnsyncedExpensePayers() =>
      (select(expensePayers)..where((t) => t.isSynced.equals(false))).get();
  Future<List<TourMember>> getUnsyncedTourMembers() =>
      (select(tourMembers)..where((t) => t.isSynced.equals(false))).get();
  Future<List<Settlement>> getUnsyncedSettlements() =>
      (select(settlements)..where((t) => t.isSynced.equals(false))).get();
  Future<List<ProgramIncome>> getUnsyncedProgramIncomes() =>
      (select(programIncomes)..where((t) => t.isSynced.equals(false))).get();

  // Normal Fetchers (Filter out deleted)
  Stream<List<Tour>> watchAllTours() =>
      (select(tours)..where((t) => t.isDeleted.equals(false))).watch();
  Future<List<Tour>> getAllTours() =>
      (select(tours)..where((t) => t.isDeleted.equals(false))).get();
  Future<List<Expense>> getExpensesByTour(String tourId) => (select(expenses)
  ...
  Future<List<JoinRequest>> getJoinRequestsByTour(String tourId) =>
      (select(joinRequests)..where((t) => t.tourId.equals(tourId))).get();
  ...
  Future<void> updateMealCount(String tourId, String userId, double count) {
    return (update(tourMembers)
          ..where((t) => t.tourId.equals(tourId) & t.userId.equals(userId)))
        .write(TourMembersCompanion(
            mealCount: Value(count), isSynced: const Value(false)));
  }
  ...
  Future<void> updateMemberRole(String tourId, String userId, String role) {
    return (update(tourMembers)
          ..where((t) => t.tourId.equals(tourId) & t.userId.equals(userId)))
        .write(TourMembersCompanion(
            role: Value(role), isSynced: const Value(false)));
  }
  ...
  Future<List<MealRecord>> getMealRecordsForDate(String tourId, DateTime date) {
    ...
    return (select(mealRecords)
          ..where((t) =>
              t.tourId.equals(tourId) & t.date.isBetweenValues(start, end)))
        .get();
  }
  ...
  Future<void> setTourLocalOnly(String tourId, bool localOnly) async {
    final key = '$_localOnlyTourKeyPrefix$tourId';
    if (localOnly) {
      await setSyncMetadata(key, '1');
    } else {
      await (delete(syncMetadata)..where((m) => m.key.equals(key))).go();
    }
  }

  Future<bool> isTourLocalOnly(String tourId) async {
    final key = '$_localOnlyTourKeyPrefix$tourId';
  ...
  Future<Set<String>> getLocalOnlyTourIds() async {
    final rows = await (select(syncMetadata)
          ..where((m) =>
              m.key.like('$_localOnlyTourKeyPrefix%') & m.value.equals('1')))
        .get();
    return rows
        .map((r) => r.key.replaceFirst(_localOnlyTourKeyPrefix, ''))
